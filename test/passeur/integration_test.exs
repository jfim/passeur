defmodule Passeur.IntegrationTest do
  ...
  describe "OAuth 2.1 authorization code flow with PKCE" do
    ...
    test "authorize without session redirects to login", %{
      client_id: client_id,
      code_challenge: code_challenge
    } do
      query =
    ...
    test "full flow: login -> authorize -> token exchange", %{
      ...
      token_conn =
        post("/oauth/token", token_body, content_type: "application/x-www-form-urlencoded")
  ...
  describe "MCP endpoint" do
    ...
    test "full flow: DCR -> login -> authorize -> token -> MCP tool call" do
      ...
      %{"code" => code} =
        auth_conn |> location() |> URI.parse() |> Map.get(:query) |> URI.decode_query()
      ...
      {:ok, init_resp} =
        Finch.build(:post, "#{base_url}/mcp", headers, init_body)
      ...
      {:ok, notif_resp} =
        Finch.build(:post, "#{base_url}/mcp", notif_headers, notif_body)
        |> Finch.request(OpenIDHttpClient)
      ...
      {:ok, tool_resp} =
        Finch.build(:post, "#{base_url}/mcp", notif_headers, tool_body)
        |> Finch.request(OpenIDHttpClient)
